...
public class MRU extends PaginacaoMemoriaBase implements PaginacaoMemoria {

	public MRU(List<Integer> stringRef, Integer frames) {
	...
	private int buscarPaginaMaisAntiga(int pointCutIndex) {
		...
		for (int pageIndex = 0; pageIndex < getPagina().getPalavras().size(); pageIndex++) {
			...
			for (int stringRefIndex = 0; stringRefIndex <= pointCutIndex; stringRefIndex++) {
	...
	public String algoritmoNome() {
		return PaginacaoMemoriaAlgoritmo.MRU.getNome();
	}
