...
public class DiscoFacade implements Serializable {
	...
	public ArrayList<HashMap<String, Object>> executar(
			final List<EscalonadorDiscoAlgoritmo> algs,
			final LinkedList<Disco> requisicoes, final Disco cabeca,
			final int modo) {
		...
		if (algs != null && !algs.isEmpty() && algs.size() == modo) {
			...
			for (EscalonadorDiscoAlgoritmo alg : algs) {
				...
				HashMap<String, Object> resultado = gerarResultado(alg, req,
						cabeca);
	...
	private HashMap<String, Object> gerarResultado(
			EscalonadorDiscoAlgoritmo algoritmo, LinkedList<Disco> requisicoes,
			Disco cabeca) {
		Class<?> klass = new Mirror().reflectClass(enumToClass
				.extractClassFromEnum(algoritmo));
		EscalonadorDisco disco = (EscalonadorDisco) new Mirror().on(klass)
				.invoke().constructor().withArgs(requisicoes, cabeca);
	...
	private HashMap<String, Object> mapResultado(EscalonadorDisco escalonador) {
		...
		mapearResultados.put("movimentoTotalCilindros",
				escalonador.movimentoTotalCilindros());
		mapearResultados.put("resultados", escalonador.resultados());
		mapearResultados.put("algoritmoNome", escalonador.algoritmoNome());
		mapearResultados.put("totalRequisicoes",
				escalonador.getTotalRequisicoes());
