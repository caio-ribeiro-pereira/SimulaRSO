...
public class ProcessoFacade implements Serializable {
	...
	public ArrayList<HashMap<String, Object>> executar(
			final List<EscalonadorProcessoAlgoritmo> algs,
		...
		if (algs != null && !algs.isEmpty() && algs.size() == modo) {
			...
			for (EscalonadorProcessoAlgoritmo alg : algs) {
				ArrayList<Processo> processos = (ArrayList<Processo>) pr
						.clone();
				HashMap<String, Object> resultado = gerarResultado(alg,
						processos, qt);
	...
	private HashMap<String, Object> gerarResultado(
			EscalonadorProcessoAlgoritmo algoritmo,
			ArrayList<Processo> processos, int quantum) {
		Class<?> klass = new Mirror().reflectClass(enumToClass
				.extractClassFromEnum(algoritmo));
		EscalonadorProcesso escalonador = (EscalonadorProcesso) new Mirror()
				.on(klass).invoke().constructor().withArgs(processos, quantum);
		return gerarResultadoMap(escalonador);
	}

	private HashMap<String, Object> gerarResultadoMap(
			EscalonadorProcesso escalonador) {
		...
		resultado.put("tempoTurnAroundMedio",
				escalonador.tempoTurnAroundMedio());
