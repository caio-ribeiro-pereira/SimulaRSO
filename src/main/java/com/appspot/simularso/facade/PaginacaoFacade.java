...
public class PaginacaoFacade implements Serializable {
	...
	public ArrayList<HashMap<String, Object>> executar(
			final List<PaginacaoMemoriaAlgoritmo> algs,
	...
	private HashMap<String, Object> gerarResultado(
			PaginacaoMemoriaAlgoritmo algoritmo,
			List<Integer> stringReferencia, Integer frames) {
		Class<?> klass = new Mirror().reflectClass(enumToClass
				.extractClassFromEnum(algoritmo));
		PaginacaoMemoria paginacao = (PaginacaoMemoria) new Mirror().on(klass)
				.invoke().constructor().withArgs(stringReferencia, frames);
	...
	private HashMap<String, Object> gerarResultadoMap(PaginacaoMemoria paginacao) {
		...
		resultado.put("totalStringReferencia", paginacao.stringReferencia()
				.size());
